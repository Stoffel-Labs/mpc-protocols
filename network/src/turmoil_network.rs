...
pub struct TurmoilNetwork {
    ...
    /// Outgoing TCP streams to other nodes (Wrapped in Arc<Mutex> to avoid map deadlocks)
    node_conns: Mutex<HashMap<PartyId, Arc<Mutex<OutgoingConn>>>>,

    /// Outgoing TCP streams to clients (Wrapped in Arc<Mutex> to avoid map deadlocks)
    client_conns: Mutex<HashMap<ClientId, Arc<Mutex<OutgoingConn>>>>,

    /// Channel receiver for all incoming messages from the background accept task
...
impl TurmoilNetwork {
    ...
    /// Initializes the network for a specific NODE. 
    /// Listener setup is now handled externally by the test harness.
    pub fn new_node(
        my_id: PartyId,
        total_nodes: usize,
    ...
    /// Initializes the network for a specific CLIENT. 
    /// Listener setup is now handled externally by the test harness.
    pub fn new_client(
        my_id: ClientId,
        total_nodes: usize,
    ...
    /// Pulls the next message from the background channel.
    pub async fn recv(&self) -> Option<Vec<u8>> {
        let mut rx = self.message_receiver.lock().await;
        rx.recv().await
    }

    async fn send_internal(&self, peer_id: PartyId, message: &[u8]) -> Result<usize, NetworkError> {
        // 1. Lock map, get or insert connection, clone Arc, and DROP map lock.
        let conn_arc = {
            ...
            if !conns.contains_key(&peer_id) {
                ...
                let stream = TcpStream::connect(&addr)
                    .await
                    .map_err(|_| NetworkError::PartyNotFound(peer_id))?;
                conns.insert(
                    peer_id,
                    Arc::new(Mutex::new(Framed::new(stream, LengthDelimitedCodec::new()))),
                );
            }

            conns.get(&peer_id).unwrap().clone()
        }; // HashMap lock is released here!

        // 2. Lock only the individual connection during the await point
    ...
    async fn send_to_client_internal(
        ...
        let conn_arc = {
            ...
            if !conns.contains_key(&client_id) {
                ...
                let stream = TcpStream::connect(&addr)
                    .await
                    .map_err(|_| NetworkError::ClientNotFound(client_id))?;
                conns.insert(
                    client_id,
                    Arc::new(Mutex::new(Framed::new(stream, LengthDelimitedCodec::new()))),
                );
            ...
            conns.get(&client_id).unwrap().clone()
        }; // HashMap lock is released here!
...
impl TurmoilTestHarness<'_> {
        ...
        Fut: Future<Output = Result<(), Box<dyn std::error::Error>>> + Send + 'static,
    {
        ...
        self.sim.client(format!("client_{}", cid), async move {
            // 1. Bind listener and spawn background task ON THE CLIENT HOST
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    tokio::select! {
                        ...
                        res = incoming.next(), if has_streams => {
                            match res {
                                Some(Ok(bytes)) => {
            ...
            // 2. Pass the receiver into the synchronous network constructor
            let network = Arc::new(TurmoilNetwork::new_client(cid, total_nodes, rx));
        ...
        Fut: Future<Output = Result<(), Box<dyn std::error::Error>>> + Send + 'static,
    {
        ...
        self.sim.host(format!("node_{}", id), move || {
            let node_logic = node_logic
                .lock()
                .unwrap()
                .take()
                .expect("rebooting nodes not supported");

            async move {
                // 1. Bind listener and spawn background task ON THE NODE HOST
                let listener = TcpListener::bind("0.0.0.0:8080").await.unwrap();
                let (tx, rx) = unbounded_channel();
                ...
                // 2. Pass the receiver into the synchronous network constructor
                let network = Arc::new(TurmoilNetwork::new_node(id, total_nodes, rx));
    ...
    fn test_node_broadcast() {
        ...
        for id in 1..=2 {
            harness.add_node(id, move |network| async move {
                ...
                network
                    .send_to_client(99, ack_msg.as_bytes())
                    .await
                    .unwrap();
                Ok(())
            });
