...
impl<F: PrimeField> KOrCLNode<F> {
    ...
    pub async fn run<N: Network + Send + Sync>(
        ...
        // `try_finalize` indexes `r_prime_bits[0..m]` on the message-processing
        // path, where an out-of-bounds would panic the receive task instead of
        // returning. Reject a short prep here, as KOrCS does for its own.
        if prandm.r_prime_bits.len() < m {
            return Err(KOrCLError::LengthError);
        }
