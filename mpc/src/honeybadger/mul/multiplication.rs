...
// received
async fn finalize_mul<F: FftField>(storage: &MultStorage<F>) -> Result<Vec<RobustShare<F>>, MulError> {
...
async fn reconstruct_masked_values<F: FftField>(received_shares: &HashMap<PartyId, (Vec<RobustShare<F>>, Vec<RobustShare<F>>)>, share_len: usize, n: usize) -> Result<(Vec<F>, Vec<F>), InterpolateError> {
...
impl<F: FftField, R: RBC> Multiply<F, R> {
    ...
    pub async fn init<N: Network + Send + Sync>(
        ...
        // find chunks that have not been opened yet by other nodes
        ...
        // need this if open_mult_handler will not be called again to try reconstruction
        if storage.received_shares.len() >= 2 * self.t + 1 && storage.openings.is_none() {
            ...
            match reconstruct_masked_values(&storage.received_shares, share_len, self.n).await {
                Ok(openings) => {
        ...
        if have_batch_recon1.iter().all(|b| *b) && have_batch_recon2.iter().all(|b| *b) && storage.openings.is_some() {
            let shares_mult = finalize_mul(&storage).await?;
    ...
    pub async fn open_mult_handler(
        ...
        // Store the a-x, b-y values opened through batch reconstruction in the appropriate slot
        ...
        // if needed, reconstruct the remaining a-x,b-y values from the shares sent via RBC
        if storage.received_shares.len() >= 2 * self.t + 1 && storage.openings.is_none() {
            // `share_len != 0`, since some honest nodes have sent us their shares
            info!("Received enough messages with shares to try reconstruction using RBC");
            let openings = reconstruct_masked_values(&storage.received_shares, share_len, self.n).await?;
