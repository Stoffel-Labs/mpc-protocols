//! Statistics instrumentation for HoneyBadgerMPC.
//!
//! Enabled only with the `statistics` cargo feature. Provides:
//!
...
impl fmt::Display for NodeStatisticsSnapshot {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        row!(
            "PRandBitD",
            self.sent.prand_bit_d,
            self.received.prand_bit_d
        );
        row!(
            "PRandBitDEcho",
            self.sent.prand_bit_d_echo,
            self.received.prand_bit_d_echo
        );
        write!(
            f,
            "└───────────────────────────────────────┴──────────┴──────────┘"
...
pub(crate) fn record_received(msg: &WrappedMessage, counts: &DirectionalMsgCounts) {
    ...
    use crate::honeybadger::{
        batch_recon::BatchReconMsgType, double_share::DouShaPayload,
        ran_dou_sha::messages::RanDouShaPayload, share_gen::RanShaMessageType,
    };
...
impl<N: Network + Send + Sync> Network for CountingNetwork<N> {
    ...
    async fn send(&self, recipient: PartyId, message: &[u8]) -> Result<usize, NetworkError> {
        self.counters.record_outbound(message, 1);
    ...
    async fn broadcast(&self, message: &[u8]) -> Result<usize, NetworkError> {
        ...
        self.counters.record_outbound(message, n);
