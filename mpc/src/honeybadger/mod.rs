...
pub struct HoneyBadgerMPCNodeOpts {
    ...
    /// Target number of ready PreMulC offline-phase bundles to keep in the
    /// pool (see `premulc_pk`). 0 (the default) means FpDiv preprocessing is
    /// never generated by `run_preprocessing`.
    pub n_premulc: usize,
    /// The `pk` size each pooled PreMulC bundle is generated at. Must equal
    /// `k - 1` of whatever `SecretFixedPoint` precision `div_fixed` is
    /// called with, since a node only supports one fixed-point precision at
    /// a time (see the global `SecretFixedPoint` precision singleton).
    pub premulc_pk: usize,
...
impl HoneyBadgerMPCNodeOpts {
    ...
    /// Configures `run_preprocessing` to keep `n_premulc` PreMulC offline-phase
    /// bundles ready, each sized at `premulc_pk` (= k-1 of the fixed-point
    /// precision `div_fixed` will be called with).
    pub fn set_premulc_target(&mut self, n_premulc: usize, premulc_pk: usize) {
        self.n_premulc = n_premulc;
        self.premulc_pk = premulc_pk;
    }
    /// Configures `run_preprocessing` to keep `n_zero_shares` degree-2t
    /// zero-sharings ready in the pool. Must cover at least
    /// `n_premulc * premulc_pk` for `ensure_premulc_shares` to succeed, since
    ...
    R: RBC<Id = SessionId>,
{
    ...
    async fn div_fixed(
        ...
        // Check preprocessing inventory, topping up (via run_preprocessing,if short. This relies on
        // self.params.n_triples/n_random_shares/n_prandbit/n_prandint
        // already being configured generously enough to cover one FpDiv
        // call.
        // Two PreMulC offline-phase bundles are also needed
        let (triples_needed, random_shares_needed, prandbit_needed, prandint_needed) =
            fpdiv_prep_counts(k, f);
        let mut rng = StdRng::from_rng(OsRng).unwrap();
        let short = {
            let store = self.preprocessing_material.lock().await;
            let len = store.length();
            len.beaver_triples < triples_needed
                || len.random_shr < random_shares_needed
        ...
        let prep = {
            ...
            let bitdec_suf_mul_inv_prep = store.take_premulc_prep()?;
            let sufor_prep = store.take_premulc_prep()?;
    ...
    R: RBC<Id = SessionId>,
{
    ...
    /// Ensure the `premulc_preps` pool has `self.params.n_premulc` bundles
    /// ready (each sized `self.params.premulc_pk`).
        ...
        N: Network + Send + Sync + 'static,
    {
        let no_have = {
            let store = self.preprocessing_material.lock().await;
            store.length().premulc
        };
        if no_have >= self.params.n_premulc {
            info!("There are enough PreMulC preps");
            return Ok(());
        }
        let missing = self.params.n_premulc - no_have;
        let pk = self.params.premulc_pk;
