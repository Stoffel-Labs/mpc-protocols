...
#[derive(Debug, Clone)]
pub struct PRandBitDNode<F: PrimeField> {
    pub id: usize,
    pub n: usize,
    pub t: usize,
    pub l: usize,
    pub k: usize,
    pub store: Arc<Mutex<HashMap<SessionId, Arc<Mutex<PRandBitDStore<F>>>>>>,
}

impl<F: PrimeField> PRandBitDNode<F> {
    ...
    pub async fn generate_riss<N: Network>(
        ...
        for tset in tsets {
            let r_t_i = rand::thread_rng().gen_range(-bound, bound + 1);
    ...
    pub async fn riss_handler<N: Network>(
        ...
        if store.r_t.len() == total_tsets {
            ...
            let poly_fq = self.build_all_f_polys(store.r_t.clone()).await?;
    ...
    pub async fn output_handler<N: Network>(
        ...
        if store.share_r_plus_b.len() >= needed {
            ...
            let my_r0_share = store.share_r_2.ok_or_else(|| PRandError::NotSet)?;
            let lsb_elem = F2_8::from(lsb as u8);
            let my_b2_share = my_r0_share + (lsb_elem);

            info!(node_id = self.id, "Generated [b]_2 share");
    ...
    pub async fn build_all_f_polys(
        &mut self,
        tsets: HashMap<Vec<usize>, i64>,
    ) -> Result<HashMap<Vec<usize>, DensePolynomial<F>>, PRandError> {
        tsets
            .into_iter()
            .map(|(tset, _)| {
                // Construct interpolation points
                let xs = std::iter::once(F::zero())
                    .chain(tset.iter().map(|&j| F::from((j + 1) as u64)))
                    .collect::<Vec<_>>();
                let ys = std::iter::once(F::one())
                    .chain(std::iter::repeat(F::zero()).take(tset.len()))
                    .collect::<Vec<_>>();
