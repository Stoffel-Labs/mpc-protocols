...
//!
//! Note that step 3 returns while the protocol is still running. `bin_values` keeps filling as
//! messages arrive, and callers are expected to wait on it — SBV-Broadcast's step 2 is literally
//! `wait until bin_values != {}` — rather than treat what step 3 hands back as the final answer.
//!
//! # Implementation status
//!
//! The message handling now follows the algorithm above, but one gap remains: nothing signals
//! `bin_values` growing, and it has no accessor, so callers cannot yet observe the set that step 3
//! hands back. See the note on the `output` field of [`BvBroadcast`].
...
struct BvBroadcastStore {
    ...
    /// The set that accumulates the values in step 8, i.e. the output of the protocol.
...
pub struct BvBroadcast<I> {
    ...
    /// Channel announcing the sessions whose `bin_values` has changed, so that callers blocked on a
    /// set that fills asynchronously (step 3) can be woken.
    ///
    /// TODO: never sent on, and `bin_values` has no accessor, so callers currently have no way to
    /// observe the set filling.
    output: Sender<I>,
}
...
impl BvBroadcast<AvssSessionId> {
    ...
    /// (step 2) and returning the current contents of `bin_values` (step 3).
    ///
    /// As in the paper, this returns before the protocol has finished, and the returned set has not
    /// necessarily reached its final value — in practice it is empty, since no `BVAL` can have been
    /// counted yet. Callers must wait for `bin_values` to fill rather than treat this as the
    /// outcome.
    pub async fn init<N>(
        &self,
        session_id: AvssSessionId,
        ...
        v: u8,
        network: Arc<N>,
    ) -> Result<HashSet<u8>, BvBroadcastError>
    where
        N: Network + Send + Sync,
    {
        ...
        let store = self.get_or_create_store(session_id, self.id).await;
        let bin_values = store.lock().await.bin_values.clone();
        return Ok(bin_values);
    }
        ...
        N: Network + Send + Sync,
    {
        ...
        if let TaggedMessage::Binary(recv_v) = message.payload {
            ...
            if n_recv_v == self.threshold + 1 {
                ...
                if recv_bit == 0 {
                    store.lock().await.bit_sent.0 = true;
                } else {
                    store.lock().await.bit_sent.1 = true;
                }
                network.broadcast(&enc_bit).await?;
            } else if n_recv_v == 2 * self.threshold + 1 {
                store.lock().await.bin_values.insert(recv_v);
            }
        }
