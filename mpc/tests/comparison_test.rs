...
// Mirrors spawn_bitdec_receiver_tasks in bitwise_test.rs exactly, one level
// deeper
// LTZ adds no session routing of its own, it only drives PreMod2m's.
...
// ── LTZ e2e ──────────────────────────────────────────────────────────────────
//
// LTZ(a, k) = -Trunc(a, k, k-1), reused here via PreMod2m(a, k, m=k-1)
// directly instead of a dedicated Mod2m/Trunc chain. Checks the reconstructed
// output is 1 for u_bar < 0 and 0 otherwise.

async fn ltz_run(u_bar: i128, k: usize, dp_bits: usize) {
    let n = 5;
    let t = 1;
    ...
    for (i, p) in prep.into_iter().enumerate() {
        ...
        init_set.spawn(async move { node.run(a, k, p, session, net, duration).await.unwrap() });
    }
    let mut s_shares = Vec::with_capacity(n);
    while let Some(r) = init_set.join_next().await {
        s_shares.push(r.unwrap());
    }
...
async fn ltz_zero() {
    setup_tracing();
    ltz_run(0, 8, 40).await;
}

#[tokio::test]
async fn ltz_most_negative() {
    setup_tracing();
    // boundary of Z<8> = [-128, 127]
    ltz_run(-128, 8, 40).await;
}

#[tokio::test]
async fn ltz_max_positive() {
    setup_tracing();
    ltz_run(127, 8, 40).await;
}

#[tokio::test]
async fn ltz_minus_one() {
    setup_tracing();
    ltz_run(-1, 8, 40).await;
}
