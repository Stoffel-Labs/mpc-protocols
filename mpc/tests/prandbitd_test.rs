...
async fn prandbitd_correctness_e2e() {
    ...
    let (protocol_output_int_tx, _protocol_output_int_rx) = mpsc::channel(128);
    let (protocol_output_bit_tx, _protocol_output_bit_rx) = mpsc::channel(128);
    let mut nodes: Vec<PRandBitDNode<GoldilocksField, Fr>> = (0..num_parties)
        .map(|i| {
            PRandBitDNode::new(
                ...
                num_parties,
                threshold,
                protocol_output_bit_tx.clone(),
                protocol_output_int_tx.clone(),
            )
            .unwrap()
        })
        .collect();
    ...
    for node in &mut nodes {
        node.generate_riss(
            session_id.clone(),
            small_field_bits[node.id].clone(),
            l,
            k,
            batch_size,
            network.clone(),
    ...
    for node in &mut nodes {
        ...
        evaluation_points.push(binary_domain.element(node.id));
    }
    ...
    for idx_share in 0..batch_size {
        ...
        let (_, value_int) = RobustShare::recover_secret(&shares_int, num_parties).unwrap();
